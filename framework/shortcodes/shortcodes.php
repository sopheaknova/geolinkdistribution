	...
	*/
	if ( ! function_exists( 'sp_shortcode_column' ) ) {
		function sp_shortcode_column( $atts, $content = null ) {
			...
			//output
			$out = '<div class="column ' . $size . $last . '">' . wpautop( do_shortcode( $content ) ) . '</div>';
			return $out;
		}
	} // /sp_shortcode_column
			...
			if ( $logos->have_posts() ) {
				...
				//if no description (no shortcode content)
					...
					while ( $logos->have_posts() ) : //output post content
						...
						$link   = ( sp_meta_option( 'client-link' ) ) ? ( esc_url( sp_meta_option( 'client-link' ) ) ) : ( null );
						...
						$out   .= '<div class="item column col-1' . $cols . ' no-margin">';
							...
							$out .= '<img src="' . esc_url( sp_meta_option( 'client-logo' ) ) . '" alt="' . get_the_title() . '" title="' . get_the_title() . '"' . $grayscale .' />';
							$out .= ( $link ) ? ( '</a>' ) : ( '' );
						$out   .= '</div>';
					endwhile;
			...
			if ( $testimonials->have_posts() ) {
				...
				while ( $testimonials->have_posts() ) :
					...
					if (
						...
						$out .= '<blockquote>' . apply_filters( 'sp_default_content_filters', $quote[0] ) . '</blockquote>';

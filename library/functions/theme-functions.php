...
function sp_body_class( $classes ) {
	...
	if ( is_single() || is_page() || is_singular('portfolio') ) {
		...
		} elseif ( ('2cr' == $has_sidebar) || is_category() || is_archive()) {
			$classes[] = 'sidebar-right';
		} elseif ( ('1col' == $has_sidebar) || is_search() ) {
			$classes[] = 'full-width';
		}
	}
...
function sp_clients_carousel( $posts_per_page = 12 ) {
	...
	ob_start();
	if ($post_list && $post_list->have_posts()) {
		$output .= '<div id="clients-carousel" class="flexslider">';
		...
		$output .= '<img src="' . sp_post_thumbnail('large') . '" alt="' . get_the_title() . '" />';
		$output .= '</div>';
		$output .= '</li>';	
		endwhile;
...
/* ---------------------------------------------------------------------- */
/*	Get Products subpage
/* ---------------------------------------------------------------------- */

function sp_child_page_lists_basic( $page_id, $menu_order='menu_order', $img_width, $img_height, $col = 'list' ) {
	...
	$output .= '<ul class="subpage-' . $col . '">';
	
	foreach ($pages as $page):
	$thumb = wp_get_attachment_image_src( get_post_thumbnail_id($page->ID), 'portfolio-2col');
	
	$output .= '<li>';
	$output .= '<div class="two-fourth"><div class="post-thumbnail">';
	$output .= '<a href="'.get_permalink().'"><img src="' . $thumb[0] . '" width="'. $img_width .'" height="'. $img_height .'" alt="'. $page->post_title .'" /></a>';
	$output .= '</div></div>';
...
if ( ! function_exists( 'highlight_search_excerpt' ) ) :
	function highlight_search_excerpt() {
		...
		$excerpt = sp_excerpt_string_length(250);
		$keys = implode( '|', explode( ' ', get_search_query() ) );
		$keys = preg_quote( $keys );
